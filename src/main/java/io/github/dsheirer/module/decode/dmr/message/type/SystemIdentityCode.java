...
public class SystemIdentityCode
{
    ...
    public PAR getPAR()
    {
        if(hasPAR())
        {
            return PAR.fromValue(getMessage().getInt(PAR_SUBFIELD, mOffset));
        }
    ...
    {
        return Model.fromValue(getMessage().getInt(MODEL, mOffset));
    }
    ...
    public DMRNetwork getNetwork()
    {
        if(mNetwork == null)
        {
            ...
            {
                case TINY:
                    mNetwork = new DMRNetwork(getMessage().getInt(TINY_NET, mOffset));
                    break;
                case SMALL:
                    mNetwork = new DMRNetwork(getMessage().getInt(SMALL_NET, mOffset));
                    break;
                case LARGE:
                    mNetwork = new DMRNetwork(getMessage().getInt(LARGE_NET, mOffset));
                    break;
                case HUGE:
                    mNetwork = new DMRNetwork(getMessage().getInt(HUGE_NET, mOffset));
                    break;
            }
        }
    ...
    public DMRSite getSite()
    {
        if(mSite == null)
        {
            ...
            {
                case TINY:
                    mSite = new DMRSite(getMessage().getInt(TINY_SITE, mOffset));
                    break;
                case SMALL:
                    mSite = new DMRSite(getMessage().getInt(SMALL_SITE, mOffset));
                    break;
                case LARGE:
                    mSite = new DMRSite(getMessage().getInt(LARGE_SITE, mOffset));
                    break;
                case HUGE:
                    mSite = new DMRSite(getMessage().getInt(HUGE_SITE, mOffset));
                    break;
            }
        }
